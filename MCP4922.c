...
/**
  * @brief  Envoie une valeur 12 bits à un canal spécifique du DAC MCP4922.
  * @param  channel: Le canal à utiliser (DAC_CHANNEL_A ou DAC_CHANNEL_B)
  * @param  data: La valeur 12 bits à envoyer (0-4095)
  * @retval None
  */
/*void MCP4922_Write(uint8_t channel, uint16_t data){
	// 1. Construire la trame de 16 bits
	// Bits 15-12: Configuration
	// Bit 15: 0 = Canal A, 1 = Canal B
	// Bit 14: 1 = Vref est bufferisé
	// Bit 13: 1 = Gain 1x (0 = 2x)
	// Bit 12: 1 = Sortie active (0 = Shutdown)
	//
	// Config pour Canal A (0b0111...) = 0x7000
	// Config pour Canal B (0b1111...) = 0xF000
	uint16_t command_word;
	uint16_t config_bits = (channel == DAC_CHANNEL_A) ? 0x7000 : 0xF000;

	command_word = config_bits | (data & 0x0FFF); // Masquer les 4 bits supérieurs de dat

	// 2. Activer la ligne CS (Chip Select)
	HAL_GPIO_WritePin(CS_DAC_GPIO_Port, CS_DAC_Pin, GPIO_PIN_RESET);

	// 3. Envoyer la trame via SPI
	HAL_SPI_Transmit(&hspi1, (uint8_t*)&command_word, 1, HAL_MAX_DELAY);

	// 4. Désactiver la ligne CS
	HAL_GPIO_WritePin(CS_DAC_GPIO_Port, CS_DAC_Pin, GPIO_PIN_SET);

	// 5. Mettre à jour la sortie en tirant LDAC bas puis haut
	HAL_GPIO_WritePin(LDAC_DAC_GPIO_Port, LDAC_DAC_Pin, GPIO_PIN_RESET);
	HAL_GPIO_WritePin(LDAC_DAC_GPIO_Port, LDAC_DAC_Pin, GPIO_PIN_SET);
}*/
